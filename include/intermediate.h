...
struct column_t {
  ...
  public:
    ...
    /* append consecutive values efficiently using memcpy */
    inline void append_consecutive(const value_t *src, size_t count) {
        size_t remaining = count;
        size_t src_idx = 0;

        /* pre-allocate all needed pages */
        size_t pages_needed =
        ...
        while (remaining > 0) {
            ...
            /* memcpy batch of value_t structs */
            memcpy(&pages[page_idx]->data[offset], &src[src_idx],
                   batch * sizeof(value_t));

            /* update bitmap if sparse */
            if (!direct_access) {
    ...
    inline void append_null() {
        if (direct_access) {
            direct_access = false;
            /* reserve bitmap based on page capacity to avoid reallocations */
            size_t expected_capacity = pages.capacity() * CAP_PER_PAGE;
            bitmap.reserve((expected_capacity + 63) / 64);
