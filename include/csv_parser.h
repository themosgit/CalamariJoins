...
public:
  ...
  CSVParser(char escape = '"', char sep = ',', bool has_trailing_comma = false)
      : escape_(escape), comma_(sep), has_trailing_comma_(has_trailing_comma) {}
...
private:
  ...
  // true means # commas = # columns and the last comma in each line is followed
  // by the record seperator; false means # commas + 1 = # columns
