...
struct Plan {
  ...
  size_t new_join_node(bool build_left, size_t left, size_t right,
                       size_t left_attr, size_t right_attr,
                       std::vector<std::tuple<size_t, DataType>> output_attrs) {
    JoinNode join{
...
template <class T> struct ColumnInserter {
  ...
  ColumnInserter(Column &column) : column(column) { bitmap.resize(PAGE_SIZE); }
