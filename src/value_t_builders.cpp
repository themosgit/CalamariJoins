      ...
      for (auto *page_obj : column.pages) {
        ...
        for (uint16_t i = 0; i < num_rows; ++i) {
          if (get_bitmap(bitmap, i)) {
            result_col.append({data_begin[data_idx++], 0, 0, 0});
          } else {
    ...
    case DataType::VARCHAR: {
      ...
      for (auto *page_obj : column.pages) {
        ...
        if (num_rows == 0xffff) {
          ...
          auto *bitmap = reinterpret_cast<uint8_t *>(page + PAGE_SIZE -
                                                     (num_rows + 7) / 8);
          uint16_t offset_idx = 0;
          for (uint16_t i = 0; i < num_rows; ++i) {
            if (get_bitmap(bitmap, i)) {
              result_col.append({page_idx, table_idx,
    ...
    case DataType::VARCHAR: {
      ...
      auto save_long_string = [&column](const std::string &str) {
        ...
        while (offset < str.size()) {
          ...
          *reinterpret_cast<uint16_t *>(page + 2) =
              static_cast<uint16_t>(page_data_len);
      ...
      for (size_t row_idx = 0; row_idx < src_column.row_count(); ++row_idx) {
        ...
        if (!val_ptr) {
          ...
          if (val_ptr->offset == UINT16_MAX) {
            ...
            auto *p =
                reinterpret_cast<uint8_t *>(src_col.pages[page_idx]->data);
            ...
            while (page_idx < src_col.pages.size()) {
              ...
              if (*reinterpret_cast<uint16_t *>(p) != 0xfffe)
                break;
            ...
            char *char_begin =
                reinterpret_cast<char *>(page + 4 + num_valid * 2);

            uint16_t end_off = offset_array[val_ptr->offset];
            uint16_t start_off =
                (val_ptr->offset == 0) ? 0 : offset_array[val_ptr->offset - 1];
