...
std::vector<uint8_t>
Comparison::eval(const std::vector<const InnerColumnBase *> &table) const {
    auto *c = table[column];
    switch (c->type) {
    case DataType::INT32: {
        ...
        if (op == IS_NULL) {
            ...
            switch (op) {
            case EQ:
                return column->equal(comp_value);
            case NEQ:
                return column->not_equal(comp_value);
            case LT:
                return column->less(comp_value);
            case GT:
                return column->greater(comp_value);
            case LEQ:
                return column->less_equal(comp_value);
            case GEQ:
                return column->greater_equal(comp_value);
            default:
                unreachable();
            }
        }
    ...
    case DataType::VARCHAR: {
        ...
        if (op == IS_NULL) {
            ...
            case GEQ:
                return column->greater_equal(comp_value);
            case LIKE:
                return column->like(comp_value);
            case NOT_LIKE:
                return column->not_like(comp_value);
            default:
                unreachable();
            }
        }
    ...
    if (op == LIKE || op == NOT_LIKE) {
        ...
        if (record_num.has_value() && comp_num.has_value()) {
            ...
            const std::string *record_str =
                std::get_if<std::string>(&record_data);
            const std::string *comp_str = std::get_if<std::string>(&comp_value);
            if (record_str && comp_str) {
                switch (op) {
                case EQ:
                    return *record_str == *comp_str;
                case NEQ:
                    return *record_str != *comp_str;
                case LT:
                    return *record_str < *comp_str;
