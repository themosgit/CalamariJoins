...
using FilterMapType =
    std::unordered_map<TableEntity, std::unique_ptr<Statement>>;
using JoinGraphType = std::unordered_map<
    TableEntity,
    std::unordered_map<TableEntity, std::tuple<std::string, std::string>>>;
using ColumnMapType =
    std::unordered_map<TableEntity, std::unordered_map<std::string, size_t>>;
...
template <> struct fmt::formatter<hsql::ExprType> {
    ...
    auto format(hsql::ExprType value, FormatContext &ctx) const {
        static std::array<std::string_view, 17> names{
            ...
            "kExprLiteralNull",  "kExprLiteralDate",   "kExprLiteralInterval",
            "kExprStar",         "kExprParameter",     "kExprColumnRef",
            "kExprFunctionRef",  "kExprOperator",      "kExprSelect",
            "kExprHint",         "kExprArray",         "kExprArrayIndex",
            "kExprExtract",      "kExprCast",
        };
        return fmt::format_to(ctx.out(), "{}", names[int(value)]);
    }
};

template <> struct fmt::formatter<hsql::OperatorType> {
    template <class ParseContext> constexpr auto parse(ParseContext &ctx) {
        return ctx.begin();
    }
...
std::tuple<std::string, TableEntity> extract_column_and_table(
    hsql::Expr *expr, const std::unordered_map<std::string, int> &table_counts,
    ...
    if (expr->hasTable()) {
        ...
        if (auto itr = alias_map.find(table); itr != alias_map.end()) {
        ...
        } else if (auto itr = table_counts.find(table);
            ...
            if (count == 1) {
                ...
                throw std::runtime_error(
                    fmt::format("Ambiguous table: {}", table));
            }
        } else {
            throw std::runtime_error(
                fmt::format("Unknown table name: {}", table));
        }
    } else {
        if (auto itr = column_to_tables.find(column);
            itr != column_to_tables.end()) {
            if (itr->second.size() > 1) {
                throw std::runtime_error(fmt::format(
                    "Ambiguous column: {0}, {1} have column {0}", column,
                    itr->second | views::join(", "s) |
                        ranges::to<std::string>()));
            } else {
                auto &table_name = itr->second[0];
                if (auto itr = table_counts.find(table_name);
                    itr != table_counts.end()) {
                    auto count = itr->second;
                    if (count == 1) {
...
void assert_column(hsql::Expr *expr) {
    if (expr->type != hsql::kExprColumnRef) {
        throw std::runtime_error(fmt::format(
            "left side of \"Equals\" condition must be a ColumnRef"));
        exit(1);
    }
}

void insert_filter(FilterMapType &filters, TableEntity entity,
                   std::unique_ptr<Statement> stmt) {
    if (auto itr = filters.find(entity); itr == filters.end()) {
        filters.emplace(std::move(entity), std::move(stmt));
    } else {
        auto new_stmt =
            LogicalOperation::makeAnd(std::move(itr->second), std::move(stmt));
...
void parse_expr_impl(
    hsql::Expr *expr, const std::unordered_map<std::string, int> &table_counts,
    const std::unordered_map<std::string, std::vector<std::string>>
        &column_to_tables,
    const AliasMapType &alias_map, const ColumnMapType &column_map,
    FilterMapType &filters, DSU &join_union,
    std::unique_ptr<Statement> &out_statement, TableEntity &out_entity,
    int level = 0) {
    namespace views = ranges::views;
    switch (expr->type) {
    case hsql::kExprOperator: {
        ...
        case hsql::kOpOr: {
            ...
            parse_expr_impl(expr->expr, table_counts, column_to_tables,
                            alias_map, column_map, filters, join_union, left,
                            left_entity, level + add);
            // fmt::println("parse right");
            parse_expr_impl(expr->expr2, table_counts, column_to_tables,
                            alias_map, column_map, filters, join_union, right,
                            right_entity, level + add);
            if (level == 0 and op_type == hsql::kOpAnd) {
                // fmt::println("Top level And");
                if (left) {
                    // fmt::println("insert left");
                    // left->pretty_print();
                    insert_filter(filters, std::move(left_entity),
                                  std::move(left));
                }
                if (right) {
                    // fmt::println("insert right: {}", (void*)right.get());
                    // right->pretty_print();
                    insert_filter(filters, std::move(right_entity),
                                  std::move(right));
                }
            } else {
                if (!left or !right) {
                    throw std::runtime_error("Non top level contains join "
                                             "condition instead of filter");
                }
                if (left_entity != right_entity) {
                    throw std::runtime_error("Filter can not be pushed down");
                }
                if (op_type == hsql::kOpAnd) {
                    out_statement = LogicalOperation::makeAnd(std::move(left),
                                                              std::move(right));
                } else {
                    out_statement = LogicalOperation::makeOr(std::move(left),
                                                             std::move(right));
                }
        ...
        case hsql::kOpNot: {
            ...
            parse_expr_impl(expr->expr, table_counts, column_to_tables,
                            alias_map, column_map, filters, join_union, child,
                            child_entity, level + 1);
        ...
        case hsql::kOpNotEquals: {
            ...
            // case hsql::kOpGreaterEq: fmt::println("operator GreaterEq");
            // break; case hsql::kOpEquals:    fmt::println("operator Equals");
            // break; case hsql::kOpNotEquals: fmt::println("operator Not
            // Equals"); break; default:                 std::unreachable();
            // }
            switch (op_type) {
            case hsql::kOpLess:
                op = Comparison::Op::LT;
                break;
            case hsql::kOpLessEq:
                op = Comparison::Op::LEQ;
                break;
            case hsql::kOpGreater:
                op = Comparison::Op::GT;
                break;
            case hsql::kOpGreaterEq:
                op = Comparison::Op::GEQ;
                break;
            case hsql::kOpEquals:
                op = Comparison::Op::EQ;
                break;
            case hsql::kOpNotEquals:
                op = Comparison::Op::NEQ;
                break;
            default:
                unreachable();
            }
            ...
            assert_column(left);
            auto [left_column, left_entity] = extract_column_and_table(
                left, table_counts, column_to_tables, alias_map);
            ...
            case hsql::kExprColumnRef: {
                auto [right_column, right_entity] = extract_column_and_table(
                    right, table_counts, column_to_tables, alias_map);
                if (op_type != hsql::kOpEquals) {
                    throw std::runtime_error(
                        "Non-EuqalJoins are not supported");
                }
                size_t left_column_idx, right_column_idx;
                if (auto itr = column_map.find(left_entity);
                    itr != column_map.end()) {
                    if (auto iter = itr->second.find(left_column);
                        iter != itr->second.end()) {
                        left_column_idx = iter->second;
                    } else {
                        throw std::runtime_error(
                            fmt::format("No column: {} in table: {}",
                                        left_column, left_entity));
                    }
                } else {
                    throw std::runtime_error(
                        fmt::format("No  table: {}", left_entity));
                }
                if (auto itr = column_map.find(right_entity);
                    itr != column_map.end()) {
                    if (auto iter = itr->second.find(right_column);
                        iter != itr->second.end()) {
                        right_column_idx = iter->second;
                    } else {
                        throw std::runtime_error(
                            fmt::format("No column: {} in table: {}",
                                        right_column, right_entity));
                    }
                } else {
                    throw std::runtime_error(
                        fmt::format("No  table: {}", right_entity));
                }
            ...
            default:
                throw std::runtime_error(fmt::format(
                    "Expression type: {} not processed", right->type));
            }
            if (right->type != hsql::kExprColumnRef) {
                out_statement = std::make_unique<Comparison>(
                    column_idx(left_column, left_entity), op, std::move(value));
        ...
        case hsql::kOpBetween: {
            ...
            for (auto [idx, item] : list | views::enumerate) {
                switch (item->type) {
                case hsql::kExprLiteralInt: {
                    // fmt::println("item {}: int literal: {}", idx,
                    // item->ival);
                    values[idx] = item->ival;
                    break;
                }
                case hsql::kExprLiteralString: {
                    // fmt::println("item {}: string literal: {}", idx,
                    // item->name);
                ...
                default:
                    throw std::runtime_error(fmt::format(
                        "Expression type: {} not processed", item->type));
                }
            }
            auto stmt1 = std::make_unique<Comparison>(
                column_idx(left_column, left_entity), Comparison::Op::GEQ,
                std::move(values[0]));
            auto stmt2 = std::make_unique<Comparison>(
                column_idx(left_column, left_entity), Comparison::Op::LEQ,
                std::move(values[1]));
            out_statement =
                LogicalOperation::makeAnd(std::move(stmt1), std::move(stmt2));
            out_entity = std::move(left_entity);
            break;
        }
        case hsql::kOpIn: {
            ...
            for (auto [idx, item] : list | views::enumerate) {
                ...
                if (not out_statement) {
                    out_statement = std::make_unique<Comparison>(
                        column_idx(left_column, left_entity),
                        Comparison::Op::EQ, std::move(value));
                } else {
                    auto new_stmt = std::make_unique<Comparison>(
                        column_idx(left_column, left_entity),
                        Comparison::Op::EQ, std::move(value));
                    out_statement = LogicalOperation::makeOr(
                        std::move(out_statement), std::move(new_stmt));
                }
            }
            out_entity = std::move(left_entity);
            break;
        ...
        case hsql::kOpIsNull: {
            ...
            assert_column(child);
            auto [child_column, child_entity] = extract_column_and_table(
                child, table_counts, column_to_tables, alias_map);
            // fmt::println("child_column: {}", child_column);
            // fmt::println("child_table: {}", child_table);
            out_statement = std::make_unique<Comparison>(
                column_idx(child_column, child_entity), Comparison::Op::IS_NULL,
                std::monostate{});
            out_entity = std::move(child_entity);
            break;
        }
        default: {
            throw std::runtime_error(
                fmt::format("Operator type: {} not processed", op_type));
        }
        }
        break;
    }
    default: {
        throw std::runtime_error(
            fmt::format("Expression type: {} not processed", expr->type));
    }
    }
}
...
void parse_expr(hsql::Expr *expr,
    ...
    parse_expr_impl(expr, table_counts, column_to_tables, alias_map, column_map,
                    filters, join_union, top_statement, top_entity);
    if (top_statement) {
...
struct ParsedSQL {
    ...
    ParsedSQL(const std::unordered_map<std::string, std::vector<std::string>>
                  &column_to_tables)
    ...
    std::string executed_sql(const std::string &sql) {
        ...
        auto select_list =
            output_attrs |
            views::transform(
                [this](const std::tuple<TableEntity, std::string> &attr) {
                    auto &[entity, column] = attr;
                    if (auto itr = entity_to_alias.find(entity);
                        itr != entity_to_alias.end()) {
        ...
        if (from_clause_begin == std::string::npos) {
            ...
            if (from_clause_begin == std::string::npos) {
                throw std::runtime_error(
                    "Cannot find \"FROM\" or \"from\" in sql");
            }
        }
        ...
        std::string_view other = std::string_view{
            sql.data() + from_clause_begin,
    ...
    void parse_sql(const std::string &sql, std::string_view name) {
        ...
        if (not sql_result.isValid()) {
            throw std::runtime_error(
                fmt::format("Error parsing SQL: {}", name));
        }
        auto statement =
            (const hsql::SelectStatement *)sql_result.getStatement(0);
        ...
        for (auto table : *fromTableList) {
            ...
            auto [itr, _] = column_map.emplace(
                entity, std::unordered_map<std::string, size_t>{});
            if (auto attr_itr = attributes_map.find(entity.table);
                ...
                throw std::runtime_error(
                    fmt::format("No table: {} in schema", entity.table));
            }
        ...
        for (auto *expr : selectList) {
            ...
            case (hsql::kExprFunctionRef): {
                for (auto *child : *expr->exprList) {
                    ...
                    auto [column, entity] = extract_column_and_table(
                        child, table_counts, column_to_tables, alias_map);
            ...
            case (hsql::kExprColumnRef): {
                auto [column, entity] = extract_column_and_table(
                    expr, table_counts, column_to_tables, alias_map);
                output_attrs.emplace_back(entity, column);
                break;
            }
            default: {
                throw std::runtime_error(fmt::format(
                    "Not supported expression type in select list: {}.",
                    expr->type));
                break;
            }
            }
        ...
        parse_expr(condition, table_counts, column_to_tables, alias_map,
                   column_map, filters, join_union);

        std::unordered_map<size_t, std::vector<size_t>> joined_columns;
        for (size_t i = 0; i < column_count; ++i) {
            auto set = join_union.find(i);
            if (auto itr = joined_columns.find(set);
                itr != joined_columns.end()) {
        ...
        for (auto &[set, items] : joined_columns) {
            for (size_t i = 0; i < items.size() - 1; ++i) {
                for (auto j = i + 1; j < items.size(); ++j) {
                    ...
                    if (auto itr = join_graph.find(left_entity);
                        itr != join_graph.end()) {
                        if (auto iter = itr->second.find(right_entity);
                            iter != itr->second.end()) {
                            throw std::runtime_error(
                                "At least two conditions between a same pair "
                                "of tables.");
                        }
                        itr->second.emplace(
                            right_entity,
                        ...
                        std::unordered_map<TableEntity,
                                           std::tuple<std::string, std::string>>
                            adj_item;
                        adj_item.emplace(right_entity,
                                         std::tuple{left_column, right_column});
                        join_graph.emplace(left_entity, std::move(adj_item));
                    }
                    if (auto itr = join_graph.find(right_entity);
                        itr != join_graph.end()) {
                        itr->second.emplace(
                            left_entity, std::tuple{right_column, left_column});
                    } else {
                        std::unordered_map<TableEntity,
                                           std::tuple<std::string, std::string>>
                            adj_item;
                        adj_item.emplace(left_entity,
                                         std::tuple{right_column, left_column});
...
Plan load_join_pipeline(const json &node, const ParsedSQL &parsed_sql,
    ...
    static std::unordered_set<std::string_view> other_operators{"Aggregate",
                                                                "Gather"};
    static std::unordered_set<std::string_view> join_types{
        "Nested Loop", "Hash Join", "Merge Join"};
    static std::unordered_set<std::string_view> scan_types{"Seq Scan",
                                                           "Index Only Scan"};
    Plan ret;

    auto extract_entities =
        ...
        if (auto itr = other_operators.find(node_type);
            itr != other_operators.end()) {
            return extract_entities(extract_entities, node["Plans"][0]);
        } else if (auto itr = join_types.find(node_type);
                   itr != join_types.end()) {
            if (node_type != "Hash Join") {
                throw std::runtime_error("Not Hash Join");
            }
            auto left_type =
                node["Plans"][0]["Node Type"].get<std::string_view>();
            auto right_type =
                node["Plans"][1]["Node Type"].get<std::string_view>();
            std::unordered_set<TableEntity> left_entities, right_entities;
            if (left_type == "Hash" and right_type != "Hash") {
                left_entities = extract_entities(extract_entities,
                                                 node["Plans"][0]["Plans"][0]);
                right_entities =
                    extract_entities(extract_entities, node["Plans"][1]);
            } else if (left_type != "Hash" and right_type == "Hash") {
                left_entities =
                    extract_entities(extract_entities, node["Plans"][0]);
                right_entities = extract_entities(extract_entities,
                                                  node["Plans"][1]["Plans"][0]);
            } else {
                throw std::runtime_error(
                    "Hash Join should have at least one Hash child");
            }
            left_entities.merge(std::move(right_entities));
            return left_entities;
        } else if (auto itr = scan_types.find(node_type);
                   itr != scan_types.end()) {
            ...
            if (auto itr = alias_map.find(alias); itr != alias_map.end()) {
                ...
                throw std::runtime_error(
                    fmt::format("Cannot find alias: {}", alias));
            }
            std::unordered_set<TableEntity> entities{std::move(entity)};
            return entities;
        } else {
            throw std::runtime_error(
                fmt::format("Not supported node type: {}", node_type));
        }
    };

    auto recurse =
        [&extract_entities, &ret, &table_counts = parsed_sql.table_counts,
         &alias_map = parsed_sql.alias_map, &join_graph = parsed_sql.join_graph,
         &filters = parsed_sql.filters,
         &query_name](auto &&recurse, const json &node,
                      const OutputAttrsType &required_attrs)
        -> std::tuple<
            size_t,
            std::vector<std::tuple<TableEntity, std::string, DataType>>> {
        auto node_type = node["Node Type"].get<std::string_view>();
        ...
        if (auto itr = other_operators.find(node_type);
            ...
            for (auto &entity : left_entities) {
                if (auto itr = join_graph.find(entity);
                    itr != join_graph.end()) {
            ...
            for (const auto &[required_entity, required_column] :
                 required_attrs) {
                if (auto itr = left_entities.find(required_entity);
                    itr != left_entities.end()) {
                    if (required_entity == left_entity and
                        required_column == left_column) {
                        left_attr_already_in = true;
                    }
                    left_required.emplace_back(required_entity,
                                               required_column);
                } else if (auto itr = right_entities.find(required_entity);
                           itr != right_entities.end()) {
                    if (required_entity == right_entity and
                        required_column == right_column) {
                        right_attr_already_in = true;
                    }
                    right_required.emplace_back(required_entity,
                                                required_column);
                } else {
                    throw std::runtime_error(
                        "Required attributes cannot be found in "
                        "neither left child nor right "
                        "child.");
                }
            }
            ...
            std::tie(left, left_columns) =
                recurse(recurse, *pleft, left_required);
            std::tie(right, right_columns) =
                recurse(recurse, *pright, right_required);
            size_t idx = 0;
            ...
            for (const auto &[required_entity, required_column] :
                ...
                if (not found) {
                    throw std::runtime_error(
                        fmt::format("Cannot found the required attr: {}.{} in "
                                    "children's output",
                                    required_entity, required_column));
                }
            }
            auto new_node_id =
                ret.new_join_node(build_left, left, right, left_attr,
                                  right_attr, std::move(output_attrs));
            ...
            if (not node.contains("Alias")) {
                if (node.contains("Relation Name")) {
                    auto relation_name =
                        node["Relation Name"].get<std::string>();
                    if (auto itr = table_counts.find(relation_name);
                        ...
                        if (itr->second == 1) {
                            ...
                            throw std::runtime_error(
                                fmt::format("Table {} is used at least twice",
                                            relation_name));
                        }
                    } else {
                        throw std::runtime_error(fmt::format(
                            "Cannot find table: {}", relation_name));
                    }
                } else {
                    throw std::runtime_error("Neither \"Alias\" nor \"Relation "
                                             "Name\" exists in scan node");
                }
            } else {
                auto alias = node["Alias"].get<std::string>();
                if (auto itr = alias_map.find(alias); itr != alias_map.end()) {
            ...
            if (auto itr = attributes_map.find(entity.table);
                itr != attributes_map.end()) {
                pattributes = &itr->second;
            } else {
                throw std::runtime_error(fmt::format(
                    "Cannot find attributes for table: {}", entity.table));
            }
            ...
            std::string filename =
                "cache/" + entity.table + fmt::format("_{}.tbl", fnvhash);

            /* Dump file if its never seen before */
            uint64_t new_input_id;
            if (!fs::exists(fs::path(filename))) {
                auto table = Table::from_csv(
                    *pattributes,
                    fs::path("imdb") / fmt::format("{}.csv", entity.table),
                    filter);

                std::cout << "Dumping table to cache: " << filename
                          << std::endl;
                ...
                std::cout << "Table already in cache: " << filename
                          << std::endl;

#elif defined TEAMOPT_USE_DUCKDB
            auto table = Table::from_csv(
                *pattributes,
                fs::path("imdb") / fmt::format("{}.csv", entity.table), filter);
            ...
            for (const auto &[required_entity, required_column] :
                ...
                for (const auto &attribute : *pattributes) {
                    if (entity == required_entity and
                        attribute.name == required_column) {
                        output_columns.emplace_back(
                            required_entity, required_column, attribute.type);
            ...
            auto new_node_id =
                ret.new_scan_node(new_input_id, std::move(output_attrs));
            return {new_node_id, std::move(output_columns)};
        } else {
            throw std::runtime_error(
                fmt::format("Not supported node type: {}", node_type));
    ...
    std::tie(ret.root, std::ignore) =
        recurse(recurse, node, parsed_sql.output_attrs);
    return ret;
}
...
bool equal(const duckdb::LogicalType &lhs, DataType rhs) {
    ...
    case LogicalTypeId::INTEGER:
        return rhs == DataType::INT32;
    case LogicalTypeId::BIGINT:
        return rhs == DataType::INT64;
    case LogicalTypeId::DOUBLE:
        return rhs == DataType::FP64;
    case LogicalTypeId::VARCHAR:
        return rhs == DataType::VARCHAR;
    default:
        throw std::runtime_error(
            fmt::format("{} in DuckDB is not supported", lhs.ToString()));
    }
}

bool compare(duckdb::MaterializedQueryResult &duckdb_results,
             const ColumnarTable &results) {
    using namespace duckdb;
...
bool compare_withcache(const ColumnarTable &expected,
                       const ColumnarTable &results) {
    auto num_rows = results.num_rows;
    auto num_cols = results.columns.size();
    ...
    auto task = [&](size_t begin, size_t end) {
        for (size_t i = begin; i < end; ++i) {
            booleans[i] =
                uint8_t(results_table.table()[i] == expected_table.table()[i]);
        }
    };
    filter_tp.run(task, num_rows);
...
run(const std::unordered_map<std::string, std::vector<std::string>>
        &column_to_tables,
    std::string_view name, std::string sql, const json &plan_json,
#ifdef TEAMOPT_USE_DUCKDB
    ...
    auto plan =
        load_join_pipeline(plan_json["Plan"], parsed_sql, std::string{name});
    ...
    if (!fs::exists(fs::path(filename))) {
        ...
        fmt::println("Table already in cache: {}\n", filename);
#endif
    ...
    fmt::println(
        "Query {} >> \t\t Runtime: {} ms - Result correct: {}", name,
...
int main(int argc, char *argv[]) {
    ...
    try {
        if (argc < 2) {
            // We accept several plans to run. For now, the last one of them
            // might be `output_filename`  in which case we write the runtime
            // result to this file.
            fmt::println(stderr,
        ...
        for (auto &[table_name, attributes] : attributes_map) {
            for (auto &[_, attribute_name] : attributes) {
                if (auto itr = column_to_tables.find(attribute_name);
                    itr == column_to_tables.end()) {
                    column_to_tables.emplace(
                        attribute_name, std::vector<std::string>{table_name});
                } else {
        ...
        for (auto argument_id = int{0}; argument_id < argc; ++argument_id) {
            if (argument_id == argc - 1 &&
                std::string{argv[argument_id]} == output_filename) {
        ...
        for (const auto &[name, plan_json] : views::zip(names, plans)) {
            if ((argc < 3 ||
                 (selected_plans.find(name) != selected_plans.end())) ||
                (argc == 3 && std::string{argv[2]} == output_filename)) {
                auto sql_path =
                    fs::path(sql_directory) / fmt::format("{}.sql", name);
