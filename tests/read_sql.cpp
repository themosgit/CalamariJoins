...
void parse_expr_impl(
  ...
  case hsql::kExprOperator: {
    ...
    case hsql::kOpOr: {
      ...
      parse_expr_impl(expr->expr, table_counts, column_to_tables, alias_map,
                      column_map, filters, join_union, left, left_entity,
                      level + add);
      // fmt::println("parse right");
      parse_expr_impl(expr->expr2, table_counts, column_to_tables, alias_map,
                      column_map, filters, join_union, right, right_entity,
                      level + add);
    ...
    case hsql::kOpNot: {
      ...
      parse_expr_impl(expr->expr, table_counts, column_to_tables, alias_map,
                      column_map, filters, join_union, child, child_entity,
                      level + 1);
    ...
    case hsql::kOpIn: {
      ...
      for (auto [idx, item] : list | views::enumerate) {
        ...
        if (not out_statement) {
          out_statement = std::make_unique<Comparison>(
              column_idx(left_column, left_entity), Comparison::Op::EQ,
              std::move(value));
        } else {
...
struct ParsedSQL {
  ...
  std::string executed_sql(const std::string &sql) {
    ...
    std::string_view other =
        std::string_view{sql.data() + from_clause_begin,
  ...
  void parse_sql(const std::string &sql, std::string_view name) {
    ...
    for (auto *expr : selectList) {
      ...
      default: {
        throw std::runtime_error(fmt::format(
            "Not supported expression type in select list: {}.", expr->type));
        break;
      }
      }
    }
    ...
    parse_expr(condition, table_counts, column_to_tables, alias_map, column_map,
               filters, join_union);
      ...
      for (size_t i = 0; i < items.size() - 1; ++i) {
        for (auto j = i + 1; j < items.size(); ++j) {
          ...
          if (auto itr = join_graph.find(right_entity);
              itr != join_graph.end()) {
            itr->second.emplace(left_entity,
...
Plan load_join_pipeline(const json &node, const ParsedSQL &parsed_sql,
  ...
  auto recurse =
      ...
      -> std::tuple<
          size_t, std::vector<std::tuple<TableEntity, std::string, DataType>>> {
    auto node_type = node["Node Type"].get<std::string_view>();
    ...
    if (auto itr = other_operators.find(node_type);
      ...
      for (const auto &[required_entity, required_column] : required_attrs) {
        if (auto itr = left_entities.find(required_entity);
          ...
          throw std::runtime_error("Required attributes cannot be found in "
      ...
      auto new_node_id = ret.new_join_node(build_left, left, right, left_attr,
        ...
        if (node.contains("Relation Name")) {
          ...
          if (auto itr = table_counts.find(relation_name);
            ...
            if (itr->second == 1) {
              ...
              throw std::runtime_error(fmt::format(
                  "Table {} is used at least twice", relation_name));
            }
          } else {
            throw std::runtime_error(
      ...
      if (!fs::exists(fs::path(filename))) {
        ...
        std::cout << "Table already in cache: " << filename << std::endl;

#elif defined TEAMOPT_USE_DUCKDB
      auto table = Table::from_csv(
          *pattributes, fs::path("imdb") / fmt::format("{}.csv", entity.table),
          filter);
      ...
      for (const auto &[required_entity, required_column] : required_attrs) {
        ...
        for (const auto &attribute : *pattributes) {
          if (entity == required_entity and attribute.name == required_column) {
            output_columns.emplace_back(required_entity, required_column,
                                        attribute.type);
...
int main(int argc, char *argv[]) {
  ...
  try {
    if (argc < 2) {
      ...
      // `output_filename`  in which case we write the runtime result to this
      // file.
      fmt::println(stderr, "{} <path to plans> [<name of selected sql>] [{}]",
    ...
    for (const auto &[name, plan_json] : views::zip(names, plans)) {
      if ((argc < 3 || (selected_plans.find(name) != selected_plans.end())) ||
        ...
        const auto [result_is_correct, query_runtime] = run(
            column_to_tables, name, std::move(sql), plan_json, conn, context);
#else
